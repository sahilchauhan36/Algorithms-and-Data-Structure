# This program explains the program to find loop starting element in the given linked list.
...
class linked_list:
  ...
  def loop_first_element(self):
    """
    This module is used to find loop's first element in linked list
    """
    current_pointer = self.head 
    fast_pointer = self.head

    # Check if the Linked List is actually having Loop before proceeding further.
    if self.is_having_loop() is False:
      print("Linked List is not having any loop present.")


    while fast_pointer.next != None and current_pointer != None:
      # If Fast and Slow Pointer meets, It means there is loop present in linked list

      fast_pointer = fast_pointer.next.next
      current_pointer = current_pointer.next

      if fast_pointer is current_pointer:
        break;
      
    current_pointer = self.head
    # loop to find first element of the linked list
    while fast_pointer.next != None and current_pointer != None:
      fast_pointer = fast_pointer.next.next
      current_pointer = current_pointer.next
...
if __name__=='__main__':
  ...
  result = ll_obj.loop_first_element()
  print("4. Loop Detected on Element: %s" % result)
  ...
  # Check if loop is present in linked list
  result = ll_obj_2.loop_first_element()
  print("4. Loop Detected on Element: %s" % result)
